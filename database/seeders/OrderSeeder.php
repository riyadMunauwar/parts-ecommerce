...
class OrderSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        while($rowQty){
            ...
            $order->total_product_price = $faker->randomFloat($nbMaxDecimals = 2, $min = 10, $max = 300);
            $order->shipping_cost = $faker->randomFloat($nbMaxDecimals = 2, $min = 10, $max = 100);
            $order->user_id = $user->id;
            $order->admin_id = $admin->id;
            $order->total_vat = $faker->randomFloat($nbMaxDecimals = 2, $min = 10, $max = 100);

            $order->save();

            $orederItemQty = $faker->numberBetween($min = 1, $max =5);
